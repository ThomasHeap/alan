"""
Particle Gibbs (conditional SMC) + PGAS on the stochastic volatility model,
mirroring nonlinear_particle_gibbs.py's structure -- again only the
transition/observation densities and initial-state distribution change, CSMC
and PGAS themselves are model-agnostic. T_STEPS=100 here (vs 20 for the other
two models) specifically to let persistence-driven degeneracy show up over a
longer horizon, matching how this model is actually stressed in the PMCMC
literature.
"""
import torch as t
from sv_model import MU, X0_VAR, SIGMA_ETA, T_STEPS, f, transition_logpdf, obs_logpdf


def csmc_sweep(y, K, ref_traj, ancestor_sampling, g, T=T_STEPS):
    x = t.empty(K)
    x[0] = ref_traj[0]
    x[1:] = MU + X0_VAR ** 0.5 * t.randn(K - 1, generator=g)
    ...
    for tt in range(1, T + 1):
        ...
        x_new = t.zeros(K)
        x_new[1:] = f(x[anc[1:]], tt) + SIGMA_ETA * t.randn(K - 1, generator=g)
        x_new[0] = ref_traj[tt]
...
def run_particle_gibbs(y, n_iters, K, ancestor_sampling=False, T=T_STEPS, seed=0, init_traj=None):
    ...
    if init_traj is None:
        ...
        ref_traj[0] = MU + X0_VAR ** 0.5 * t.randn(1, generator=g).item()
        for tt in range(1, T + 1):
            ref_traj[tt] = f(ref_traj[tt - 1].item(), tt) + SIGMA_ETA * t.randn(1, generator=g).item()
    else:
...
if __name__ == "__main__":
    from sv_model import simulate, grid_filter_smoother, marginal_mean_var

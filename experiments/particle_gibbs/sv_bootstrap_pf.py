"""
Bootstrap particle filter on the stochastic volatility model in sv_model.py,
mirroring nonlinear_bootstrap_pf.py's structure. The point here isn't
multimodality (SV's latent is unimodal Gaussian) -- it's that PHI=0.98 makes
the latent strongly persistent, so a plain bootstrap filter's proposal
(sampling x_t from the prior transition, ignoring y_t) is a poor match to the
narrow, slowly-drifting filtering distribution, which is exactly the
motivation for the PMCMC line of work in the first place.
"""
import torch as t
from sv_model import MU, X0_VAR, SIGMA_ETA, T_STEPS, f, obs_logpdf, simulate, grid_filter_smoother, marginal_mean_var
...
def bootstrap_pf(y, K, T=T_STEPS, seed=0):
    ...
    x = MU + X0_VAR ** 0.5 * t.randn(K, generator=g)
    particle_history = [x.clone()]
    ancestry = []

    log_lik = 0.0
    for tt in range(1, T + 1):
        x_pred = f(x, tt) + SIGMA_ETA * t.randn(K, generator=g)
...
if __name__ == "__main__":
    ...
    for tt in [0, 1, 2, 10, 25, 50, 75, 100]:
        print(f"{tt:3d}    {mean_s[tt]:+.4f}         {pf_mean[tt]:+.4f}        {abs(mean_s[tt] - pf_mean[tt]):.4f}")
